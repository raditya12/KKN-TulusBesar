...
@section('content')
<div class="w-full h-[calc(100vh-80px)] flex flex-col md:flex-row overflow-hidden bg-white" x-data="{ sidebarOpen: false }">
    
    <!-- Mobile Sidebar Toggle -->
    <div class="md:hidden p-4 bg-white border-b border-gray-200 flex justify-between items-center z-40 shadow-sm relative">
        <h2 class="font-display-md text-xl font-bold text-[#593922]">WebGIS <span class="text-[#8c5a35] font-normal">Tulusbesar</span></h2>
        <button @click="sidebarOpen = !sidebarOpen" class="bg-primary/10 text-primary p-2 rounded-lg flex items-center gap-2 transition-colors hover:bg-primary/20">
            <span class="material-symbols-outlined" x-text="sidebarOpen ? 'close' : 'filter_list'">filter_list</span>
        </button>
    </div>

    <!-- Sidebar (Filters & Layers) -->
    <aside class="w-full md:w-[320px] lg:w-[380px] bg-white border-r border-gray-200 flex flex-col z-30 shadow-[4px_0_24px_rgba(0,0,0,0.02)] h-[calc(100vh-80px-72px)] md:h-full absolute md:relative top-[72px] md:top-0 transform transition-transform duration-300"
           :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'">
        
        <div class="p-6 border-b border-gray-100 hidden md:block">
            <h2 class="font-display-md text-2xl font-bold text-[#593922] mb-1">WebGIS <span class="text-[#8c5a35] font-light">Tulusbesar</span></h2>
        ...
        <div class="p-6 flex-grow overflow-y-auto custom-scrollbar bg-white">
            <h3 class="font-label-md font-bold text-gray-800 mb-4 uppercase tracking-wider flex items-center gap-2 text-xs">
                <span class="material-symbols-outlined text-[18px] text-primary">layers</span> Layer Peta
            </h3>

            <div class="space-y-2">
                <!-- Filter Item 1 -->
                <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white hover:bg-gray-50 cursor-pointer transition-colors border border-gray-200 shadow-sm">
                    ...
                    <input type="checkbox" checked value="Wisata" class="filter-checkbox w-5 h-5 rounded border-gray-300 text-primary focus:ring-primary accent-primary cursor-pointer">
                </label>
                ...
                <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white hover:bg-gray-50 cursor-pointer transition-colors border border-gray-200 shadow-sm">
                    ...
                    <input type="checkbox" checked value="Peternakan" class="filter-checkbox w-5 h-5 rounded border-gray-300 text-secondary focus:ring-secondary accent-secondary cursor-pointer">
                </label>

                <!-- Filter Item 3 -->
                <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white hover:bg-gray-50 cursor-pointer transition-colors border border-gray-200 shadow-sm">
                    <div class="w-9 h-9 rounded-full bg-tertiary/10 text-tertiary flex items-center justify-center shrink-0">
                        <span class="material-symbols-outlined text-[18px]">account_balance</span>
                    </div>
                    <span class="font-body-md text-gray-700 font-medium flex-grow">Fasilitas Umum</span>
                    <input type="checkbox" checked value="Fasilitas Umum" class="filter-checkbox w-5 h-5 rounded border-gray-300 text-tertiary focus:ring-tertiary accent-tertiary cursor-pointer">
                </label>

                <!-- Filter Item 4 -->
                <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white hover:bg-gray-50 cursor-pointer transition-colors border border-gray-200 shadow-sm">
                    <div class="w-9 h-9 rounded-full bg-[#d97706]/10 text-[#d97706] flex items-center justify-center shrink-0">
                        <span class="material-symbols-outlined text-[18px]">lightbulb</span>
                    </div>
                    <span class="font-body-md text-gray-700 font-medium flex-grow">PJU Bambu</span>
                    <input type="checkbox" checked value="PJU" class="filter-checkbox w-5 h-5 rounded border-gray-300 text-[#d97706] focus:ring-[#d97706] accent-[#d97706] cursor-pointer">
                </label>

                <!-- Filter Item 5 -->
                <label class="flex items-center gap-3 p-3.5 rounded-xl bg-white hover:bg-gray-50 cursor-pointer transition-colors border border-gray-200 shadow-sm">
                    <div class="w-9 h-9 rounded-full bg-[#059669]/10 text-[#059669] flex items-center justify-center shrink-0">
                    ...
                    <input type="checkbox" checked value="Sampah" class="filter-checkbox w-5 h-5 rounded border-gray-300 text-[#059669] focus:ring-[#059669] accent-[#059669] cursor-pointer">
                </label>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100">
                <div class="bg-blue-50/50 p-4 rounded-xl border border-blue-100/50">
                    ...
                    <p class="font-body-sm text-blue-900/70 text-xs leading-relaxed">Gunakan filter untuk menyaring titik lokasi peta. Data spasial diperbarui secara berkala oleh Pemdes Tulusbesar.</p>
                </div>
            </div>
        </div>
    </aside>
    ...
    <main class="flex-grow relative bg-surface-variant h-full overflow-hidden z-10">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                var layerGroups = {
                    'Wisata': L.markerClusterGroup({ maxClusterRadius: 40 }),
                    'Peternakan': L.markerClusterGroup({ maxClusterRadius: 40 }),
                    'Fasilitas Umum': L.markerClusterGroup({ maxClusterRadius: 40 }),
                    'PJU': L.markerClusterGroup({ maxClusterRadius: 40 }),
                    'Sampah': L.markerClusterGroup({ maxClusterRadius: 40 })
                };
