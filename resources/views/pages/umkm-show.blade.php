                ...
                <div class="lg:w-2/3">
                    ...
                    >
                        @if($hasMultiple)
                            ...
                            <div class="absolute inset-0 transition-opacity duration-1000 ease-in-out"
                                ...
                                <img src="{{ Str::startsWith($img, 'images/dummy/') ? asset($img) : url('/download-file?path=' . $img) }}" alt="{{ $umkm->name }} - Slide {{ $index + 1 }}" class="w-full h-full object-cover">
                            </div>
                            @endforeach
                        ...
                        @else
                            <img src="{{ empty($umkm->image_path) ? asset('images/dummy/umkm1.jpg') : (Str::startsWith($umkm->image_path, 'images/dummy/') ? asset($umkm->image_path) : url('/download-file?path=' . $umkm->image_path)) }}" alt="{{ $umkm->name }}" class="w-full h-full object-cover">
                        @endif
                    </div>
                    ...
                    <div class="mt-16 pt-8 border-t border-outline-variant/30 flex flex-col md:flex-row items-center justify-between gap-4">
                        ...
                        <div class="flex gap-2">
                            <!-- WhatsApp Share -->
                            <a href="https://api.whatsapp.com/send?text={{ urlencode('Lihat ' . $umkm->name . ' di Web Desa Tulusbesar: ' . url()->current()) }}" target="_blank" class="w-10 h-10 rounded-full bg-[#25D366]/10 border border-[#25D366]/50 flex items-center justify-center text-[#25D366] hover:bg-[#25D366] hover:text-white transition-colors" title="Bagikan ke WhatsApp">
                            ...
                            @if(isset($recommendations) && $recommendations->count() > 0)
                                ...
                                <a href="{{ route('umkm.show', $rec->slug) }}" class="group flex gap-4 items-start">
                                    <div class="w-24 h-24 shrink-0 rounded-xl overflow-hidden bg-surface-variant">
                                        <img src="{{ empty($rec->image_path) ? asset('images/dummy/umkm1.jpg') : (Str::startsWith($rec->image_path, 'images/dummy/') ? asset($rec->image_path) : url('/download-file?path=' . $rec->image_path)) }}" alt="{{ $rec->name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    </div>
                                    <div class="flex-1">
