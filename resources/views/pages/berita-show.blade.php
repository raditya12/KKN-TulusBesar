                    ...
                    >
                        @if($hasMultiple)
                            ...
                            <div class="absolute inset-0 transition-opacity duration-1000 ease-in-out"
                                ...
                                <img src="{{ Str::startsWith($img, 'images/dummy/') ? asset($img) : \Illuminate\Support\Facades\Storage::disk('public')->url($img) }}" alt="{{ $berita->title }} - Slide {{ $index + 1 }}" class="w-full h-full object-cover">
                            </div>
                            @endforeach
                        ...
                        @else
                            <img src="{{ empty($berita->image_path) ? asset('images/dummy/hero.jpg') : (Str::startsWith($berita->image_path, 'images/dummy/') ? asset($berita->image_path) : \Illuminate\Support\Facades\Storage::disk('public')->url($berita->image_path)) }}" alt="{{ $berita->title }}" class="w-full h-full object-cover">
                        @endif
                    </div>
                            ...
                            @if(isset($recommendations) && $recommendations->count() > 0)
                                ...
                                <a href="{{ route('berita.show', $rec->slug) }}" class="group flex gap-4 items-start">
                                    <div class="w-24 h-24 shrink-0 rounded-xl overflow-hidden bg-surface-variant">
                                        <img src="{{ empty($rec->image_path) ? asset('images/dummy/hero.jpg') : (Str::startsWith($rec->image_path, 'images/dummy/') ? asset($rec->image_path) : \Illuminate\Support\Facades\Storage::disk('public')->url($rec->image_path)) }}" alt="{{ $rec->title }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                                    </div>
                                    <div class="flex-1">
