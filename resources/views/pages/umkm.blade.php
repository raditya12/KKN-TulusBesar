    ...
    @php
        $slideImages = $umkms
            ...
            ->map(fn($u) => Str::startsWith($u->image_path, 'images/dummy/')
                ? asset($u->image_path)
                : url('/download-file?path=' . $u->image_path)
            )
            ->values()
            ->toArray();
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div class="bg-surface-container-lowest rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-outline-variant/30 flex flex-col group">
                    <div class="h-56 relative overflow-hidden">
                        ...
                        <img src="{{ empty($umkm->image_path) ? asset('images/dummy/umkm1.jpg') : (Str::startsWith($umkm->image_path, 'images/dummy/') ? asset($umkm->image_path) : url('/download-file?path=' . $umkm->image_path)) }}" alt="{{ $umkm->name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 sepia-[0.2]">
                    </div>
                    <div class="p-6 flex-grow flex flex-col">
                        ...
                        <p class="font-body-sm text-on-surface-variant text-justify flex-grow line-clamp-3 mb-4">
